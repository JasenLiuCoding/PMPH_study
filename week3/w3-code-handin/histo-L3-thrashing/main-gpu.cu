...
void multiStepHisto ( uint32_t* d_inp_inds
     ...
     *
     * Without these modifications, the code still runs correctly,
...
void runAll ( const uint32_t N, const uint32_t H, const uint32_t LLC ) {
    ...
    // 1. allocate memory
    uint32_t* inp_inds  = (uint32_t*) calloc(N, sizeof(uint32_t));
    float*    inp_vals  = (float*)    calloc(N, sizeof(float));
    float*    hist_gold = (float*)    calloc(H, sizeof(float));
    float*    h_hist    = (float*)    calloc(H, sizeof(float));
    ...
    { // 5. naive GPU version
        ...
        memset(h_hist, 0, H * sizeof(float));
    }
